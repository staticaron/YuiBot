...
async def get_character_details(name: str, user: Member) -> dict:
    ...
    if anilist_user is None:
        resp = requests.post(
            config.ANILIST_BASE,
            json={"query": character_query_without_stats, "variables": variables},
        )
    else:
        resp = requests.post(
            config.ANILIST_BASE,
            json={"query": character_query_with_stats, "variables": variables},
        )
...
async def get_studio_details(name: str) -> dict:
    ...
    resp = requests.post(
        config.ANILIST_BASE, json={"query": studio_query, "variables": variables}
    )
...
async def get_details_embd(data, title):
    ...
    details_embd.add_field(
        name="Average Score", value=str(data["averageScore"]) or "None", inline=True
    )

    details_embd.add_field(
        name="Mean Score", value=str(data["meanScore"]) or "None", inline=True
    )

    details_embd.add_field(
        name="Favourites", value=str(data["favourites"]) or "None", inline=True
    )

    details_embd.add_field(name="Episodes", value=str(data["episodes"]), inline=True)

    details_embd.add_field(
        name="Duration", value=str(data["duration"]) or "None", inline=True
    )
    ...
    studios = (
        data["studios"]["nodes"]
        if len(data["studios"]["nodes"]) <= 3
        else data["studios"]["nodes"][:3]
    )
    studios_str = "\n".join(
        [
            "[{name}]({url})".format(
                name=studio["name"],
                url=studio["siteUrl"] if studio["siteUrl"] is not None else "",
            )
    ...
    trailer_link = (
        "[click here](https://www.youtube.com/watch?v={})".format(data["trailer"]["id"])
        if data["trailer"] is not None and data["trailer"]["site"] == "youtube"
        else "Not Available"
    )
    ...
    try:
        ...
        status = (
            "STATUS : " + data["mediaListEntry"]["status"]
            if data["mediaListEntry"] is not None
            else ""
        )
        score = (
            (
                "🔘SCORE : " + str(data["mediaListEntry"]["score"])
                if data["mediaListEntry"]["score"] != 0
                else ""
            )
            if data["mediaListEntry"] is not None
            else ""
        ...
        total = "/" + str(
            data["mediaListEntry"]["media"]["episodes"]
            if data["mediaListEntry"] is not None
            else ""
        )

        if not (isFav == "" and status == ""):
            details_embd.set_footer(
                text="{status}{fav}{score}{progress}{total}".format(
                    status=status,
                    fav=isFav,
                    score=score,
                    progress=progress,
...
async def get_tags_embd(data, title, isAdultAllowed: bool = False) -> Embed:
    tags_embd: Embed = Embed(
        title="TAGS - " + title, color=config.NORMAL_COLOR, url=data["siteUrl"]
    )
...
async def get_anime_details_embed(name: str, user: Member) -> dict:
    data_raw = await get_media_details(name, MediaType.ANIME, user)
    data = data_raw.get("data", {}).get("Media", None)
    ...
    title = "#{id} - {eng_name} {is_adult}".format(
        ...
        is_adult=" - 18+" if data["isAdult"] else "",
    )
    ...
    return {
        "embeds": {"details": details_embd, "tags": tags_embd},
        "isAdult": data["isAdult"],
    }


async def get_manga_details_embed(name: str, user: Member) -> dict:
    ...
    embd.add_field(
        name="Favorites", value=str(data["favourites"]) or "None", inline=True
    )
    ...
    try:
        ...
        total = "/" + str(
            data["mediaListEntry"]["media"]["chapters"]
            if data["mediaListEntry"] is not None
            else ""
        )
...
async def get_character_details_embed(name: str, user: Member) -> Embed:
    ...
    data = data_raw.get("data", {}).get("Character", None)
    ...
    title = "#{} - {}".format(
        data["id"],
        data["name"]["full"]
        if data["name"]["full"] is not None
        else data["name"]["native"],
    )
    ...
    appearances_str = "\n".join(
        [
            "[{name}]({url})".format(
                ...
                else media["title"]["romaji"],
                url=media["siteUrl"] if media["siteUrl"] is not None else "",
            )
            for media in data["media"]["nodes"][:3]
        ]
    )
...
async def get_top_by_genre(genres: list, media_type: str = "ANIME") -> Scroller:
    ...
    resp = requests.post(
        ...
        json={
            ...
            "variables": {"genre": genres, "type": media_type},
        },
    ).json()
    ...
    for media in data:
        ...
        embd.description = (
            "{number}. [{name}]({link}) - {meanScore}".format(
                number=total_entry_count,
                name=title,
                link=media["siteUrl"],

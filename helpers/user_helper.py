...
async def is_following(current_user_token: str, targetID: str) -> bool:
    ...
    follow_check_variables = {"targetID": targetID}
    follow_check_response = requests.post(url=config.ANILIST_BASE, json={"query": follow_check_query, "variables": follow_check_variables}, headers={"Authorization": current_user_token}).json()
...
async def get_user_embed(targetID: str) -> Embed:
    ...
    user_resp = requests.post(url=config.ANILIST_BASE, json={"query": user_query, "variables": {"id": anilistID}}).json()
    ...
    embd: Embed = Embed(title="User Information", description="**Name** : [{name}]({url})\n**Followers** : {followers}".format(name=user_data["name"], url=user_data["siteUrl"], followers=social_data["pageInfo"]["total"]), color=config.NORMAL_COLOR)
    ...
    embd.add_field(name="Favorite Anime", value=fav_anime, inline=True)
    ...
    embd.add_field(name="Favorite Manga", value=fav_manga, inline=True)
    ...
    embd.add_field(name="Favorite Characters", value=fav_characters, inline=True)
...
async def follow_user(user: Member, target: Member) -> Embed:
    ...
    if is_already_following:
        return await general_helper.get_information_embed(title="Hold It", color=config.INFO_COLOR, description="You are already following <@{}>".format(target.id))

    # add the target to the follow list.

    variables = {"targetID": target_anilistID}

    follow_response = requests.post(url=config.ANILIST_BASE, json={"query": follow_query, "variables": variables}, headers={"Authorization": current_user_token})
    ...
    if follow_response["data"]["ToggleFollow"] is None:
        ...
        return await general_helper.get_information_embed(title="Error Occurred!", color=config.ERROR_COLOR, description="The following error occurred : ```>{}```".format("\n>".join(errors)))

    return (await general_helper.get_information_embed(title="Done", description="You are now following [{name}]({url})".format(name=follow_response["data"]["ToggleFollow"]["name"], url=follow_response["data"]["ToggleFollow"]["siteUrl"]))).set_thumbnail(url=follow_response["data"]["ToggleFollow"]["avatar"]["medium"])
...
async def unfollow_user(user: Member, target: Member) -> Embed:
    ...
    if not is_already_following:
        return await general_helper.get_information_embed(title="Hold It", color=config.INFO_COLOR, description="You are not even following <@{}>".format(target.id))

    variables = {"targetID": target_anilistID}

    unfollow_response = requests.post(url=config.ANILIST_BASE, json={"query": follow_query, "variables": variables}, headers={"Authorization": current_user_token})
    ...
    return (await general_helper.get_information_embed(title="Done", description="You are no longer following [{name}]({url})".format(name=unfollow_response["data"]["ToggleFollow"]["name"], url=unfollow_response["data"]["ToggleFollow"]["siteUrl"]))).set_thumbnail(
        url=unfollow_response["data"]["ToggleFollow"]["avatar"]["medium"]
    )
...
async def get_user_media_stats(target: Member, media_type: str = "ANIME") -> Embed:
    if media_type == "ANIME":
        stats_query = anime_stats_query
    elif media_type == "MANGA":
        stats_query = manga_stats_query

    anilistID = await general_helper.get_id_from_userID(str(target.id))

    stats_resp = requests.post(url=config.ANILIST_BASE, json={"query": stats_query, "variables": {"userID": anilistID}}).json()
    ...
    embd = Embed(title="Anime Statistics", description="**Name : ** [{}]({})\n\n".format(user_data["name"], user_data["siteUrl"]))

    embd.set_thumbnail(url=user_data["avatar"]["medium"])

    embd.add_field(name="Total Count", value=str(stats_data["count"]), inline=False)

    embd.add_field(name="Mean Score", value=str(stats_data["meanScore"]), inline=False)

    if media_type == "ANIME":
        embd.add_field(name="Standard Deviation", value=str(stats_data["standardDeviation"]), inline=False)

    if media_type == "ANIME":
        embd.add_field(name="Episodes Watched", value=str(stats_data["episodesWatched"]), inline=False)
    else:
        embd.add_field(name="Chapters Read", value=str(stats_data["chaptersRead"]), inline=False)

    if media_type == "ANIME":
        embd.add_field(name="Watch Time", value=await general_helper.get_time_str_from_seconds(stats_data["minutesWatched"] * 60, 2), inline=False)

    if media_type == "MANGA":
        embd.add_field(name="Volumes Read", value=str(stats_data["volumesRead"]), inline=False)

...
async def get_user_embed(userID: str) -> Embed:
    ...
    fav_anime = "\n".join(["[{name}]({url})".format(name=anime["title"]["english"] if anime["title"]["english"]
                          is not None else anime["title"]["romaji"], url=anime["siteUrl"]) for anime in user_data["favourites"]["anime"]["nodes"]]) + " ..."
    fav_anime = ("None" if len(
        user_data["favourites"]["anime"]["nodes"]) <= 0 else fav_anime)
    ...
    fav_manga = "\n".join(["[{name}]({url})".format(name=manga["title"]["english"] if manga["title"]["english"]
                          is not None else manga["title"]["romaji"], url=manga["siteUrl"]) for manga in user_data["favourites"]["manga"]["nodes"]]) + " ..."
    fav_manga = ("None" if len(
        user_data["favourites"]["manga"]["nodes"]) <= 0 else fav_manga)
    ...
    fav_characters = "\n".join(["[{name}]({url})".format(name=character["name"]["full"], url=character["siteUrl"])
                               for character in user_data["favourites"]["characters"]["nodes"]]) + " ..."
    fav_characters = ("None" if len(
        user_data["favourites"]["characters"]["nodes"]) <= 0 else fav_characters)
    ...
    embd.add_field(
        ...
        value="Count : **{count}**\nMean Score : **{mean_score}**\nStandard Deviation : **{standard_deviation}**\nEpisode Watched : **{episodes}**\nTop Genre : \n{genre}".format(
            ...
            genre="\n".join("{bullet}**{name}**".format(
                name=genre["genre"], bullet=config.BULLET_EMOTE) for genre in user_data["statistics"]["anime"]["genres"])
        ),
        inline=True
    )
    ...
    embd.add_field(
        ...
        value="Count : **{count}**\nMean Score : **{mean_score}**\nStandard Deviation : **{standard_deviation}**\nChapters Read : **{chapters}**\nTop Genre : \n{genre}".format(
            ...
            genre="\n".join("{bullet}**{name}**".format(
                name=genre["genre"], bullet=config.BULLET_EMOTE) for genre in user_data["statistics"]["manga"]["genres"])
        ),
        inline=True
    )
...
async def follow_user(user: Member, target: Member) -> Embed:
    ...
    return (await general_helper.get_information_embed(
        title="Done",
        description="You are now following [{name}]({url})".format(
            name=follow_response["data"]["ToggleFollow"]["name"], url=follow_response["data"]["ToggleFollow"]["siteUrl"])
...
async def unfollow_user(user: Member, target: Member) -> Embed:
    ...
    return (await general_helper.get_information_embed(
        title="Done",
        description="You are no longer following [{name}]({url})".format(
            name=unfollow_response["data"]["ToggleFollow"]["name"], url=unfollow_response["data"]["ToggleFollow"]["siteUrl"])
...
async def get_user_media_stats(target: Member, media_type: str = "ANIME") -> Embed:
    ...
    embd = Embed(
        ...
        description="**Name : ** [{}]({})\n\n".format(
            user_data["name"], user_data["siteUrl"])
    )

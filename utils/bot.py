    ...
    async def on_message(self, message: Message):
        ...
        if message.content.strip().startswith(SPOTIFY_TRACK_BASE):
            ...
            if len(track_id_match) > 0:
                links = await spotify_helper.send_alternatives(message=message, spotify_track_id=track_id_match[0])
                embd:Embed = await general_helper.get_information_embed(title="Alternate Links", description="Best Matches")
